//! Gemini Bringup SP bare-metal template.

#![no_std]
#![no_main]

// Choose the right panic backend based on features.
cfg_if::cfg_if! {
    if #[cfg(feature = "panic-itm")] {
        extern crate panic_itm;
    } else if #[cfg(feature = "panic-semihosting")] {
        extern crate panic_semihosting;
    } else if #[cfg(feature = "panic-halt")] {
        extern crate panic_halt;
    } else {
        compile_error!("Must have one of the following features enabled: \
            panic-itm, panic-semihosting, or panic-halt");
    }
}

use cortex_m_rt::pre_init;

// This line is unnecessary once you're using any of the stm32h7 crate
// explicitly, but if you remove or comment out all the code doing that, you
// need this declaration or the stm32 vector table doesn't get linked in. It's
// safe to always include it.
extern crate stm32h7;

// Shorthand alias for the particular device support module.
...
#[entry]
fn main() -> ! {
    // note: you'll want to lose the leading undescore from cp to use it
    let (_cp, p) = system_init();

    // Turn on GPIO port I, which is where the LEDs are attached (on pins 8-11).
    p.RCC.ahb4enr.modify(|_, w| w.gpioien().enabled());
    cortex_m::asm::dsb();
...
fn system_init() -> (cortex_m::Peripherals, device::Peripherals) {
    ...
    // Hand our peripheral tokens to the caller in case they want to, like,
    // twiddle some stuff
    (cp, p)
}

...
//!
//! By default, code that records the same value multiple times in a row
//! will result in a separate ring buffer entry being recorded for each value.
//! If this occurs often, it can quickly fill up the whole ring buffer with
//! repetitive data. Therefore, ring buffers of entries that implement the
//! [`PartialEq`] trait can be configured to de-duplicate repeated values by
//! incrementing a counter on the previous entry, rather than recording a new
//! entry.
//!
//! To enable entry de-duplication, add the `dedup` argument at the end of the
//! [`ringbuf!`] or [`counted_ringbuf!`] macro. For example:
//!
//! ```
//! // When recording the same `u32` value multiple times in a row,
//! // this ring buffer will increment the counter on the previous
//! // entry, rather than recording two separate ones.
//! ringbuf!(u32, 16, 0, dedup);
//! ```
//!
...
//! }
//!
//! // When this ring buffer records a `MyEvent` entry which is equal to
//! // the most recent entry, the counter on the previous entry will be
//! // incremented, rather than recording a new entry.
//! counted_ringbuf!(MyEvent, 16, MyEvent::NothingHappened, dedup);
//! ```
//!
//! Note that the per-entry counts generated by de-duplication are
//! distinct from the entry variant counters produced by [`counted_ringbuf!`].
//! A [`counted_ringbuf!`] will count the _total number of times_ a particular
//! entry variant has been recorded, coalescing those values based not on their
//! [`PartialEq`] implementation, but based on their implementation of the
//! [`counters::Count`] trait. If we consider the `MyEvent` enum in the above
//! example, two `MyEvent::SomethingElseHappened` with different `u32` values
//! would increment the _same_ total counter, because they are the same variant
//! of the enum. However, they would *not* be de-duplicated, because they are
...
#[macro_export]
macro_rules! ringbuf {
    ($name:ident, $t:ty, $n:expr, $init:expr, dedup) => {
...
macro_rules! ringbuf {
    ...
    ($t:ty, $n:expr, $init:expr, dedup) => {
        $crate::ringbuf!(__RINGBUF, $t, $n, $init, dedup);
    };
    ($t:ty, $n:expr, $init:expr) => {
        $crate::ringbuf!(__RINGBUF, $t, $n, $init);
    };
...
macro_rules! counted_ringbuf {
    ...
    ($name:ident, $t:ident, $n:expr, $init:expr, dedup) => {
        #[used]
    ...
    ($t:ident, $n:expr, $init:expr, dedup) => {
        $crate::counted_ringbuf!(__RINGBUF, $t, $n, $init, dedup);
    };
...
macro_rules! counted_ringbuf {
    ($name:ident, $t:ident, $n:expr, $init:expr, dedup) => {
        $crate::ringbuf!($name, $t, $n, $init, dedup)
    };
    ($name:ident, $t:ident, $n:expr, $init:expr) => {
        $crate::ringbuf!($name, $t, $n, $init)
    };

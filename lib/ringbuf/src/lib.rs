...
/// `__RINGBUF_COUNTS`.
///
/// Events in a counted ringbuf should be recorded using the [`event!`] macro.
#[macro_export]
...
///
/// `event!(NAME, event)` will insert `event` into the ringbuffer called
...
#[macro_export]
macro_rules! event_root {
    ($buf:ident, $event:expr) => {
        $crate::event!(crate::$buf, $event);
    };
    ($event:expr) => {
        $crate::event!(crate::__RINGBUF, $event);
    };
}
...
///
/// An array of counters which track the total number of times a ringbuf has
/// recorded each variant of its entry type.
///
/// In practice, instantiating this directly is strange -- see the
/// [`counted_ringbuf!`] macro.
///
#[derive(Debug)]
pub struct EventCounts<T: Event, const VARIANTS: usize> {
    counts: [EventCount; VARIANTS],
    _t: core::marker::PhantomData<fn(T)>,
}

///
/// A counter of the total number of times a ringbuf has recorded a particular
/// variant its entry type, along with the name of the variant.
///
/// In practice, instantiating this directly is strange -- see the
/// [`counted_ringbuf!`] macro.
///
...
/// type using the [`#[derive(Event)]`][drv] attribute.
///
/// [drv]: ringbuf_macros::Event
pub trait Event {
    /// The number of variants of this type.
    const VARIANTS: usize;

    /// The name of each of this type's variants.
    const NAMES: &'static [&'static str];

    /// Returns the index into the counters array for an instance of this type.
    ///
    /// Implementations of this method MUST return a unique index for each
    /// variant of this type, and each time this method is called for a *given*
    /// variant, the return value must be the same. Otherwise, your counters
    /// will probably end up kinda weird. Using the [`#[derive(Event)]`][drv]
    /// attribute will ensure that this is the case.
    ///
    /// [drv]: ringbuf_macros::Event
    fn index(&self) -> usize;
}
...
impl<T: Event, const VARIANTS: usize> EventCounts<T, { VARIANTS }> {
    pub const fn new(names: &[&'static str]) -> Self {
        ...
        const NEW_COUNT: EventCount = EventCount {
            name: "<uninitialized>",
            count: AtomicUsize::new(0),
        };
        assert!(names.len() == VARIANTS);

        let mut counts = [NEW_COUNT; VARIANTS];
        let mut i = 0;
        while i < VARIANTS {
    ...
    pub fn increment(&self, event: &T) {
        let idx = event.index();
        self.counts[idx].count.fetch_add(1, Relaxed);
    }
}

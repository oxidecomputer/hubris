...
impl<'a, R: Vsc7448Rw> Bsp<'a, R> {
    ...
    pub fn reinit(&mut self) -> Result<(), VscError> {
        ...
        // Disable queue system limits for our active ports.
        //
        // See https://github.com/oxidecomputer/hubris/issues/1399 for details;
        // in short, we couldn't find a queue configuration that wouldn't
        // occasionally lock up, so we're just going to disable these limits
        // entirely.  If the system gets overloaded, frames will instead be
        // "tail dropped" before being loaded into memory; this should be
        // transient, compared to the undesired behavior of queues getting
        // permanently stuck.
        for (port, _) in
            PORT_MAP.iter().enumerate().filter(|(_, c)| c.is_some())
        {
            self.vsc7448.write_with(
                vsc7448_pac::XQS().QLIMIT_PORT(port as u8).QLIMIT_DIS_CFG(),
                |r| {

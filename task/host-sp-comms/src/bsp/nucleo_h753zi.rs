...
// The Nucleo-144 has no SP3-style host interrupt pin.
// Pick an unused GPIO so the code compiles; PE2 is exposed on
// CN11 pin 62 but is not wired to anything in this configuration.
...
impl ServerImpl {
    ...
    pub(crate) fn perform_inventory_lookup(
        ...
        match index {
            0 => {
                // The service processor itself (STM32H753).

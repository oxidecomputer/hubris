...
impl idl::InOrderJefeImpl for ServerImpl<'_> {
    ...
    fn set_state(
        ...
        if self.state != state {
            ...
            // ding!
            for (task, mask) in generated::STATE_CHANGE_MAILING_LIST {
                notify_task(task, mask);
            }
        }
        Ok(())
    }
...
impl idol_runtime::NotificationHandler for ServerImpl<'_> {
    ...
    fn handle_notification(&mut self, bits: userlib::NotificationBits) {
        ...
        if bits.check_notification_mask(notifications::FAULT_MASK) {
            ...
            // dong!
            for (task, mask) in generated::FAULT_MAILING_LIST {
                notify_task(task, mask);
            }
        }
...
fn notify_task(task: hubris_num_tasks::Task, mask: u32) {

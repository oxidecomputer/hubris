...
impl idl::InOrderDrooperImpl for ServerImpl {
    fn droop(
        ...
        let (device, _rail) = i2c_config::pmbus::v12_sys_a2(I2C.get_task_id());
        let ibc = Bmr491::new(&device);

        // Droop the voltage for the requested time period in ms.
        // We pick 9V because it's approximately what we see in the field for
        // mfg-quality#140.
        let _ = ibc.set_vout(9);
        userlib::hl::sleep_for(time_ms);

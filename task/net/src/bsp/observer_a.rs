...
// TODO V2P5_PHY_A2_PG and V1P0_PHY_A2_PG are now routed to Ignition rather than
// the SP. Do we need to wait for them?
const PG_PINS: [drv_stm32xx_sys_api::PinSet; 0] = [];
...
impl bsp_support::Bsp for BspImpl {
    ...
    fn configure_ethernet_pins(sys: &Sys) {
        pins::RmiiPins {
            refclk: Port::A.pin(1), // CLK_50MHZ_RMII_KSZ8463
            crs_dv: Port::A.pin(7), // RMII_KSZ8463_TO_SP_RX_DV
            tx_en: Port::G.pin(11), // RMII_SP_TO_KSZ8463_TX_EN_
            txd0: Port::G.pin(13),  // RMII_SP_TO_KSZ8463_TXD0
            txd1: Port::G.pin(12),  // RMII_SP_TO_KSZ8463_TXD1

            rxd0: Port::C.pin(4), // RMII_KSZ8463_TO_SP_RXD0
            rxd1: Port::C.pin(5), // RMII_KSZ8463_TO_SP_RXD1
            af: Alternate::AF11,
        }
        .configure(sys);

        pins::MdioPins {
            mdio: Port::A.pin(2), // MIM_SP_TO_VSC8562_MDIO_V3P3
            mdc: Port::C.pin(1),  // MIM_SP_TO_VSC8562_MDC_V3P3
            af: Alternate::AF11,
        }
        .configure(sys);
    }
    ...
    fn new(eth: &eth::Ethernet, sys: &Sys) -> Self {
        ...
        let bsp = mgmt::Config {
            // A2_EN is now controlled by Ignition
            power_en: None,
            slow_power_en: false,
            power_good: &PG_PINS,

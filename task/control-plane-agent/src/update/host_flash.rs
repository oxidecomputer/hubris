...
impl HostFlashUpdate {
    ...
    pub(crate) fn start_hash(&self, slot: u8) -> Result<(), SpError> {
        self.task
            .hash_significant_bits(Self::slot_to_dev(slot as u16)?)
            .map_err(hf_to_gwhf)
    }
    ...
    pub(crate) fn get_hash(
        ...
        let hash = self
            .task
            .get_cached_hash(Self::slot_to_dev(slot as u16)?)
            .map_err(hf_to_gwhf)?;
        dest[..hash.len()].copy_from_slice(&hash);
        Ok(())
    }
    ...
    pub(crate) fn read_page(
        ...
        self.task
            .read(addr, &mut dest[..PAGE_SIZE_BYTES])
            .map_err(hf_to_gwhf)
    }

...
fn process_config() -> Result<Generated> {
    ...
    let region_descs = region_table
        .into_iter()
        .map(|(k, region)| fmt_region(k, &region))
        .collect();
...
fn fmt_region(key: RegionKey, region: &RegionConfig) -> TokenStream {
    let RegionConfig {
        base,
        size,
        attributes,
    ...
    if let RegionKey::Owned(i, name) = key {
        let ident: syn::Ident = syn::parse_str(
            &format!("TASK_{i}_REGION_{}", name.to_uppercase()),
        ).unwrap();
        quote::quote! {
            RegionDesc {
                base: unsafe { #ident.as_ptr() } as *mut u8,
                size: #size,
                attributes: #atts,
            }
        }

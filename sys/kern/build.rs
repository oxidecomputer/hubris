...
fn process_config() -> Result<Generated> {
    ...
    // We'll collect symbols to export as we go.
    ...
    // No symbol is recorded for the shared null region.
    ...
    for (name, region) in &kconfig.shared_regions {
        ...
        // No symbol is recorded for any shared region.
    }
    ...
    for (i, task) in kconfig.tasks.iter().enumerate() {
        for (name, region) in &task.owned_regions {
            ...
            // Now we record symbols:
            let ident = format!("TASK_{i}_REGION_{}", name.to_uppercase());
            symbols.insert(
                ident.clone(),
                region.base,
            );
            let ident = syn::parse_str::<syn::Ident>(&ident)?;
            let size = region.size as usize;
            owned_arrays.push(quote::quote! {
                static mut #ident: [core::mem::MaybeUninit<u8>; #size];
            });
        }
    }
...
fn generate_statics(gen: &Generated) -> Result<()> {
    ...
    /////////////////////////////////////////////////////////
    // Task owned memory arrays.
    let owned_arrays = &gen.owned_arrays;
    writeln!(
        file,
        "{}",
        quote::quote! {
...
fn generate_linker_script(gen: &Generated) -> Result<()> {
    let out = build_util::out_dir();
    let path = out.join("kernel-task-symbols.x");
    let mut file =
        File::create(&path).context("creating kernel-task-symbols.x file")?;

    writeln!(file, "// generated by sys/kern/build.rs")?;

    for (name, address) in &gen.symbols {
        writeln!(file, "{name} = {address:#x?};")?;
    }

    println!("cargo:rustc-link-search={}", out.display());
    println!("cargo:rustc-link-arg=kernel-task-symbols.x");
    Ok(())
}

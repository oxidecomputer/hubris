...
fn generate_statics() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for task in &kconfig.tasks {
        ...
        writeln!(file, "        ],")?;
        writeln!(file, "        entry_point: {:#08x},", task.entry_point)?;
        writeln!(file, "        initial_stack: {:#08x},", task.initial_stack)?;
    ...
    for region in &kconfig.regions {
        ...
        writeln!(file, "        base: {:#08x},", region.base)?;
        writeln!(file, "        size: {:#08x},", region.size)?;
        writeln!(
            file,
            "        attributes: unsafe {{ \

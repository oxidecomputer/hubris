...
impl Task {
    ...
    pub fn reinitialize(&mut self) {
        ...
        crate::arch::Current::reinitialize(self);
    }
    ...
    /// next return to user. This should be done "on our way out" to user code,
    /// toward the end of the syscall routine.
    ///
    /// Note that this does *not* magically run user code. This is not Unix
    /// `swtch`.
    ///
    /// # Safety
    ///
    ...
    ///
    /// 1. It will reconfigure the MPU according to instructions in `task`,
    ///    which could break kernel invariants if done maliciously.
    /// 2. It will leak a pointer to `task` into static space, where it will be
    ///    used on next kernel entry.
    ///
    ...
    pub(crate) unsafe fn switch_to(&mut self) {
        crate::arch::Current::apply_memory_protection(self);
        // Safety: our contract above is sufficient to ensure that this is safe.
        unsafe {
            crate::arch::Current::set_current_task(self);
        }
    }
}
...
pub fn select(previous: usize, tasks: &mut [Task]) -> &mut Task {
    match priority_scan(previous, tasks, |t| t.is_runnable()) {
        Some(index) => &mut tasks[index],
        None => panic!(),
    }
}
...
///
/// On success, the return value is the task's index in the task table.

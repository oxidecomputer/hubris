...
impl Config {
    ...
    fn from_file_with_hasher(
        ...
        // The manifest may also include a `fmc` key, which defines extra
        // memory-mapped peripherals attached over an FMC bus
        if let Some(fmc) = &toml.fmc {
            let Some(p) = peripherals.get(&fmc.peripheral_region) else {
                bail!(
                    "could not find peripheral region '{}'",
                    fmc.peripheral_region
                );
            };
            let base_address = p.address;
            use build_fpga_regmap::Node;

            let fmc_file = cfg.parent().unwrap().join(&fmc.register_map);
            let fmc_contents = std::fs::read(fmc_file)?;
            hasher.write(&fmc_contents);

            let root: Node =
                serde_json::from_str(std::str::from_utf8(&fmc_contents)?)
                    .with_context(|| {
                        format!(
                            "failed to read FMC register map at {:?}",
                            fmc.register_map
                        )
                    })?;
            ...
            for (i, p) in children.iter().enumerate() {
                ...
                if *addr_offset != i * 256 {
                    bail!("fmc peripherals must be spaced at 256 bytes");
                }
                peripherals.insert(
                    format!("fmc_{inst_name}"),
                    Peripheral {

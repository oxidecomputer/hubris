...
impl ImageAccess<'_> {
    /// Instantiate an ImageAccess
    ///   - `Some(flash)` if backed by flash in whole or part.
    ///   - `Some(buffer)`  if backed by a buffer in whole or part.
    ///     `buffer` must be u32 aligned and is always at image offset zero.
    ///   - `component` and `slot` identify the flash storage bank for this image.
    pub fn new<'a>(
        flash: Option<&'a drv_lpc55_flash::Flash<'a>>,
        buffer: Option<&'a [u8]>,

...
impl idl::InOrderUpdateImpl for ServerImpl<'_> {
    ...
    fn finish_image_update(
        ...
        // has some meaning.
        let range = &flash_range(component, slot).store;
...
impl ServerImpl<'_> {
    ...
    fn switch_default_boot_image(
        ...
        let stage0 = &flash_range(RotComponent::Stage0, SlotId::A).store;
    ...
    fn compare_cache_to_stage0(&self) -> Result<(), UpdateError> {
        ...
        for addr in (0u32..(clen as u32)).step_by(BYTES_PER_FLASH_PAGE) {
            ...
            if flash_page
                != cached[(addr as usize)
                    ..(addr as usize).saturating_add(BYTES_PER_FLASH_PAGE)]
            {
                return Err(UpdateError::ImageMismatch);
            }
        }
    ...
    fn cached_stage0_image_len(&self) -> Result<usize, UpdateError> {
        if let Ok(vectors) = ImageVectorsLpc55::try_from(
            ...
            let exec_range = &flash_range(RotComponent::Stage0, SlotId::A).exec;
    ...
    fn read_stage0next_to_cache(&mut self) -> Result<usize, UpdateError> {
        let stage0next = ImageAccess::new(
            Some(&self.flash),
            None,
            RotComponent::Stage0,
            SlotId::B,
    ...
    fn write_cache_to_stage0(&mut self) -> Result<(), UpdateError> {
        ...
        if span.store.end < span.store.start.saturating_add(clen as u32) {
            return Err(UpdateError::BadLength);
        }

        ...
        hl::recv(
            ...
            |cryptref, op, msg| match op {
                Operation::Encrypt => {
                    ...
                    aes.ctrl.modify(|_, w| {
                        w.new_hash().start().mode().aes().hashswpb().set_bit()
                    });
                ...
                Operation::SHA1 => {
                    // XXX Should not need this? Investigate?
                    turn_off_aes();
                    turn_on_aes();
                    let (_, caller) = msg
                        .fixed_with_leases::<[u32; 4], ()>(2)
                    ...
                    aes.ctrl.modify(|_, w| w.new_hash().start());
                    aes.ctrl.modify(
                        |_, w| w.new_hash().start().mode().sha1(), //.hashswpb().set_bit()
                    );
...
fn load_aes_block(
    aes: &device::hashcrypt::RegisterBlock,
    cdata: &mut CryptData,
) -> Option<ResponseCode> {
...
fn load_partial_hash_block(
    ...
    // Edge case: if the message length is a multiple of 512-bits  (64 bytes)
    // we have to put the final '1' in a separate block which also contains the
    // final length.
    if cdata.len % 64 == 0 {
        buf[0] = 0x80;
    }
    ...
    if actual_final {
        let len: u64 = (cdata.len * 8) as u64;
        buf[63] = ((len >> 0) & 0xff) as u8;
        buf[62] = ((len >> 8) & 0xff) as u8;
        buf[61] = ((len >> 16) & 0xff) as u8;
        buf[60] = ((len >> 24) & 0xff) as u8;
        buf[56] = ((len >> 32) & 0xff) as u8;
        buf[57] = ((len >> 40) & 0xff) as u8;
        buf[58] = ((len >> 48) & 0xff) as u8;
        buf[59] = ((len >> 54) & 0xff) as u8;
    }

    for i in 0..16 {
        unsafe {
            let a: u32 = buf[i * 4 + 0] as u32;
            let b: u32 = ((buf[i * 4 + 1] as u32) << 8) as u32;
            let c: u32 = ((buf[i * 4 + 2] as u32) << 16) as u32;
            let d: u32 = ((buf[i * 4 + 3] as u32) << 24) as u32;
            let v: u32 = a | b | c | d;
            while !aes.status.read().waiting().bit() {}
...
fn load_a_block(
    aes: &device::hashcrypt::RegisterBlock,
    c: &mut Option<CryptData>,
) {
    let cdata = if let Some(cdata) = c { cdata } else { return };
...
fn get_hash_data(
    ...
    if let Some(()) = c.caller.borrow(1).write_at::<[u32; 5]>(c.wpos, data) {
        aes.intenclr
            .write(|w| w.digest().set_bit().waiting().set_bit());
        return Some(ResponseCode::Success);
    } else {
        return Some(ResponseCode::BadArg);
    }

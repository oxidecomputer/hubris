...
impl ServerImpl {
            ...
            match hubpack::deserialize::<Header>(rx_data_buf) {
                Ok((header, data)) => {
                    ...
                    match (meta.size as usize).checked_sub(Header::MAX_SIZE) {
                        ...
                        None => {
                            ringbuf_entry!(Trace::SizeUnderflowError);
                            None
                        }
                    }
                }
    ...
    fn handle_message(
        ...
        // If the message is something we don't expect or something we cannot
        // reply to
        ...
        // the actual Message. This sends out _our_ protocol version number,
        // which may differ from the host's, but should be compatible with it.
        ...
        for module in modules.to_indices().map(LogicalPort) {
            if module <= LogicalPortMask::MAX_PORT_INDEX {
                if result.failure.is_set(module) {
                    ...
                    out[error_idx] = HwError::FpgaError as u8;
                    error_idx += 1;
                }
            } else if requested_invalid_modules.is_set(module.0).unwrap() {
                out[error_idx] = HwError::InvalidModuleIndex as u8;
                error_idx += 1;
            }
        }
    ...
    fn get_status(
        ...
        let (mod_status, mut result) = self.transceivers.get_module_status();
    ...
    fn select_page(
        ...
        if let Some(page) = page.page() {
            result.chain(self.transceivers.set_i2c_write_buffer(&[page]));
            result.chain(self.transceivers.setup_i2c_write(
                PAGE_SELECT,
                1,
                mask,
            ));
        ...
        if let Some(bank) = page.bank() {
            result.chain(self.transceivers.set_i2c_write_buffer(&[bank]));
            result.chain(self.transceivers.setup_i2c_write(
                BANK_SELECT,
                1,
                result.success,
    ...
    // Empty Ok() result if there were no I2C errors. The `LogicalPortMask`
    // returned by the Err represents which ports had I2C errors.
    ...
    fn read(
        ...
        result.chain(self.transceivers.setup_i2c_read(
            mem.offset(),
            mem.len(),
            result.success,
        ));

        let mut final_result = ModuleResult::default();
        let mut idx = 0;
            ...
            loop {
                ...
                if status & Reg::QSFP::PORT0_STATUS::BUSY == 0 {
                    ...
                    if status & Reg::QSFP::PORT0_STATUS::ERROR != 0 {
                        ...
                        final_result.failure.set(port);
                    } else {
                        // Add data to payload
                        final_result.success.set(port);

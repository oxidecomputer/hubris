...
impl From<MailboxCmd> for DecodedCommand {
    fn from(mbox_cmd: MailboxCmd) -> Self {
        let (cmd, argument, has_result) = match mbox_cmd {
            ...
            MailboxCmd::WritePkgPwrLim(lim) => (0x02, Some(lim.0), false),
            ...
            MailboxCmd::WriteDramThrottle(dram_throttle) => {
                (0x0d, Some(dram_throttle.0), false)
            }
            ...
            MailboxCmd::ReadCcdBistResult(ccd_inst) => {
                (0x14, Some(ccd_inst.0), true)
            }
            MailboxCmd::ReadCcxBistResult(ccx_inst) => {
                (0x15, Some(ccx_inst.0), true)
            }
            ...
            MailboxCmd::ReadPostCode(postcode_offset) => {
                (0x20, Some(postcode_offset.0 & 0b1111), true)
            }
            ...
            MailboxCmd::ReadPubDieSerNo(dword_offset, category, chiplet_no) => {
                ...
                let chipno = u32::from(chiplet_no.0) << 8;
...
impl SbRmi<ByteProto> {
    ...
    pub fn nthreads(&self) -> Result<u32> {
        let lo = self.read_byte_reg(Register::ThreadNumberLo)? as u32;
        let hi = self.read_byte_reg(Register::ThreadNumberHi)? as u32;
        Ok((hi << 8) + lo)
    }
...
impl<T: SbRmiMessageProto> Validate<Error> for SbRmi<T> {
    fn validate(device: &I2cDevice) -> Result<bool> {
        let sbrmi = SbRmi::<T>::new(*device)?;
...
impl Validate<Error> for Sbrmi {
    fn validate(_device: &I2cDevice) -> Result<bool> {
        Ok(true)
    }
}

...
pub mod rolling_timer {
    ...
    impl<'a> RollingTimer<'a> {
        ...
        /// Convert a rolling timer into a ptimer
        ///
        /// SAFETY: You must only ever call this function once. TIM5 must never
        /// be used again after calling `into_ptimer`.
    ...
    mod ptime {
        ...
        /// Obtain the tickrate. Currently hardcoded to 1MHz
        fn tickrate() -> u32 {
            1_000_000
        }

        /// Perform timekeeping. This checks if we need to increment the period
        /// counter, which counts the number of half-rollovers.
        fn timekeep() {
        ...
        /// The current time, expressed as u64 counts at the [`tickrate()`]
        /// frequency.
        ...
        pub(crate) static VTABLE: hubris_ptime::PTimeVTable =

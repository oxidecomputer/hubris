...
impl NotificationHandler for ServerImpl {
    ...
    fn handle_notification(&mut self, bits: userlib::NotificationBits) {
        ...
        if bits.check_notification_mask(notifications::SP_RESET_IRQ_MASK) {
            ...
            // SP_RESET.
            let _ =
                gpio.pint_clear(ROT_TO_SP_RESET_L_IN_PINT_SLOT, PintFlag::Both);
    ...
    fn do_handle_sp_reset(&mut self) -> bool {
        ...
        if let Ok(detected) = gpio.pint_detect(SLOT, PintFlag::Falling) {
            if !detected {
                ...
                //
                // Otherwise, cases where we assert SP_RESET then clean-up the
                // PINT condition will have a pending notification.
                ringbuf_entry!(Trace::SpResetNotAsserted);
                return true; // no work required.
            }
        } else {
            // Spurious notification, no work required
            return true;
        }
...
fn main() -> ! {
    ...
    let _ = Pins::from(server.gpio)
        .pint_enable(ROT_TO_SP_RESET_L_IN_PINT_SLOT, PintCondition::Falling);
    ...
    let _ = Pins::from(server.gpio)
        .pint_enable(SP_TO_ROT_JTAG_DETECT_L_PINT_SLOT, PintCondition::Falling);
    sys_irq_control(notifications::JTAG_DETECT_IRQ_MASK, true);

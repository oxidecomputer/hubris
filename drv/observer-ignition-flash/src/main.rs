...
//! Hacky temporary test of Observer's ignition flash. We should generalize the
//! ignition-flash task instead.
...
// SP_TO_IGN_TRGT_FLASH_MUX_SEL
const MUX_SELECT: sys_api::PinSet = sys_api::Port::C.pin(9);
...
fn main() -> ! {
    ...
    let dev = spi_front.device(drv_spi_api::devices::IGNITION_FLASH);
    ...
    // TODO its's deselected by default with a pulldown, so should we configure
...
impl<S: SpiServer> ServerImpl<'_, S> {
    fn set_selected(&mut self, selected: bool) {
        if self.is_selected != selected {
            self.sys.gpio_set_to(MUX_SELECT, selected);

...
// License, self. 2.0. If a copy of the MPL was not distributed with this
...
pub struct ViperPhy<'a, 'b, P>(pub &'b mut Phy<'a, P>);
impl<'a, 'b, P> Deref for ViperPhy<'a, 'b, P> {
    type Target = Phy<'a, P>;
...
impl<'a, 'b, P> DerefMut for ViperPhy<'a, 'b, P> {
...
impl<'a, 'b, P: PhyRw> ViperPhy<'a, 'b, P> {
    ...
    pub(crate) fn patch(&mut self) -> Result<(), VscError> {
        ringbuf_entry!(Trace::ViperPatch(self.port));

        self.modify(phy::STANDARD::EXTENDED_CONTROL_AND_STATUS(), |r| {
            *r = (u16::from(*r) | 1).into()
        })?;
        self.modify(phy::STANDARD::BYPASS_CONTROL(), |r| {
            *r = (u16::from(*r) | 8).into()
        })?;
        self.write(
        ...
        self.write(phy::TEST::TEST_PAGE_5(), 0x1f20.into())?;
        self.modify(phy::TEST::TEST_PAGE_8(), |r| r.0 |= 0x8000)?;
        self.write(phy::TR::TR_16(), 0xafa4.into())?;
        self.modify(phy::TR::TR_18(), |r| r.0 = (r.0 & !0x7f) | 0x19)?;
        ...
        self.write(phy::TR::TR_17(), 0xb002.into())?;
        self.write(phy::TR::TR_16(), 0x8f82.into())?;

        self.write(phy::TR::TR_18(), 0x0000.into())?;
        self.write(phy::TR::TR_17(), 0x0004.into())?;
        self.write(phy::TR::TR_16(), 0x9686.into())?;
        ...
        self.write(phy::TR::TR_18(), 0x00ff.into())?;
        self.write(phy::TR::TR_17(), 0xfaff.into())?;
        self.write(phy::TR::TR_16(), 0x8f80.into())?;

        self.write(phy::TR::TR_18(), 0x0090.into())?;
        self.write(phy::TR::TR_17(), 0x1809.into())?;
        self.write(phy::TR::TR_16(), 0x8fec.into())?;

        self.write(phy::TR::TR_18(), 0x00b0.into())?;
        self.write(phy::TR::TR_17(), 0x1007.into())?;
        ...
        self.write(phy::EXTENDED_2::CU_PMD_TX_CTRL(), 0x028e.into())?;
        ...
        self.write(phy::GPIO::GPIO_0(), 0x4098.into())?;

        // "Release 8051 SW Reset"
        self.write(phy::GPIO::GPIO_0(), 0xc098.into())?;

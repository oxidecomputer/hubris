...
1. Your task invokes `send`, naming the recipient task and operation code, and
   providing the outgoing data as a `&[u8]`. It also provides a buffer where the
   response should be deposited, as a `&mut [u8]`.
...
5. It then uses the `reply` operation (discussed below) to send a response
   message back.

6. The kernel copies the response message into the `&mut [u8]` buffer your task
   provided, fills out some additional information to inform your task that the
   message was successful, and marks your task as runnable.
...
The length gives the number of bytes in the return buffer that have been written
by the kernel and are now valid. This happens no matter what the response code
is, so an operation can return detailed data even in the case of an error.
...
related to crashing tasks. These are called "dead codes," and are described
later in this chapter. Applications should choose their response codes to avoid
colliding with them. Fortunately, they're very large numbers, so if applications
start their errors from 1 they should be safe.
...
Any leases are reliably and atomically _revoked_ when the caller resumes (i.e.
after `reply`), which means it's safe to lend out information from the caller's
stack.
...
Tasks check for notifications by calling `receive` -- the `receive` operation
takes an additional parameter called the _notification mask,_ which is a 32-bit
word. Any 1-bits in the notification mask express to the kernel that the task
would like to find out if the corresponding bit in its notification set has been
posted since it last checked.
...
2. The task immediately returns from `receive` without blocking, and

3. The result of `receive` is a _notification message_ instead of an IPC.

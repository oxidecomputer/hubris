...
waiting to receive) are interrupted and given a "dead code" to indicate that the
IPC will never complete.
...
is explicitly intended to allow tasks to keep some information from "past lives"
if required.

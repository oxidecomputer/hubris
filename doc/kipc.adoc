...
A successful call to `restart_task` has the following effects:
...
If a task asks to restart _itself,_ the kernel mumbles "`alright, your funeral`"
and restarts the caller. Given that `restart_task` is intended to be restricted
to the supervisor, and the supervisor can't `panic!` to restart without taking
out the system, this seemingly weird move may actually prove useful.
...
As with `restart_task`, it is possible for a task to use `fault_task` to fault
...
forever (or at least, until the supervisor restarts it), you can receive from
the kernel with _no_ notification mask bits set.
